...
### 主催者による募集の編集
- 主催者は自分の募集の詳細ページから「編集する」ボタンで、タイトル・エリア・ジャンル・開催日・時間帯・募集人数・参加条件・予算・ひとことを後から変更できる(募集作成と同じフォームを編集モードとして再利用する)
- **承認済み(`approved`)の参加者が1人でもいる場合は編集不可**。「募集の取り消し」と全く同じ判断基準(承認済みapplicationの有無)であり、フロントはボタンをdisabledにして理由を表示する。変更したい場合は、参加者がいなくなってから編集または取り消し・作り直しを行う運用とする
- DB側もBEFORE UPDATEトリガー(`prevent_edit_with_approved_applicants`)で同条件を担保する: title/area/genre/event_at/event_date/capacity/who/budget/noteのいずれかを変更しようとした場合に承認済み参加者が存在すれば更新を拒否する。承認による自動close・取消/退会による自動reopen・主催者によるcancelledへの更新など、statusのみを書き換える自動遷移はこのトリガーの対象外(内容が変わらないため素通りする)
...
| 募集詳細 | 基本情報、**参加メンバー**(承認済み参加者。ログイン中は誰でも閲覧可、名前タップでプロフィール閲覧)、参加表明(承認制)、承認待ち/参加確定/見送り/募集終了/取消の状態表示、主催者向け承認UI(応募者のプロフィール表示付き。承認すると承認待ちリストから参加メンバーへ移動)、主催者による「編集する」ボタン(承認済み参加者がいれば無効化。募集作成と同じフォームで内容を更新)、主催者による「募集を取り消す」ボタン(承認済み参加者がいれば無効化)、応募者による「参加を取り消す」ボタン(取り消し時に主催者宛の一言コメントを添付可)、主催者名タップでプロフィール閲覧、現地集合の案内。**公開コメント+返信・通報はPhase 2で実装** |
...
- [ ] 承認済み参加者がいない募集は、主催者が「編集する」から内容(タイトル・エリア・ジャンル・開催日・時間帯・人数・条件・予算・ひとこと)を変更でき、詳細ページに反映される
- [ ] 承認済み参加者が1人でもいる募集は、主催者でも「編集する」ボタンが無効化されて編集できない(API直叩きでもDBのトリガーにより拒否されることを確認)
