...
- **profiles**: 全ログインユーザーがselect可(公開プロフィール方針)。insert/updateは本人(`auth.uid() = id`)のみ
...
| 画面 | 主な機能 |
|---|---|
| トップ | ヒーロー(写真コラージュ+都市ラベル)、新着募集3件、ジャンルギャラリー(タップで絞り込み遷移)、安心ポイント、やくそく |
| 募集一覧 | エリア/ジャンル/人数/予算で絞り込み。募集終了はグレーのタグ表示 |
| 募集詳細 | 基本情報、参加メンバー、参加表明(承認制)、承認待ち/参加確定/募集終了の状態表示、主催者向け承認UI(応募者のプロフィール表示付き)、DM、コメント+返信(1階層)、現地集合の案内、通報 |
